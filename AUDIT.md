...
- Google URL Inspection reports all six canonical pages submitted and indexed. The sitemap was read September 19 with zero errors/warnings. Its API indexed count is not used to determine indexing. Added those six canonical URLs to GSC Wizard's Indexing Tracker and initialized their status with URL Inspection: six indexed, no warnings. This monitors status and does not submit URLs for Google indexing. Intentional HTTP, www and legacy-home redirects remain in place.
...
- Preview validation: GitHub reports deployment `5SDeas8EdeYyvgbn3eMn2nvhUXKp` successful for commit `d4517d29cfacc40584dfdb69002f359f89c7b12c`. The authenticated browser rendered all five guides and live topic headlines. The homepage rendered 18 news cards, four quotes, official coin images, the logo and the world clock. Related-topic navigation, story attribution/noindex, and FRIED pause/resume passed. English, Chinese and Spanish layouts had no horizontal overflow in 320, 390 and 768px iframe viewports; fixed headers remained at top zero. Desktop guide appearance was inspected. These are browser viewport checks, not a physical iPhone test. The temporary harness was removed after validation. Vercel API access remains unavailable, but the existing GitHub integration and authenticated preview work. Production deployment and live-domain checks follow the merge.
